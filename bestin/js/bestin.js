...
const rs485_energy_config = {
    portType: CONFIG.portType, //'socket' , 'serial'
    portName: process.platform.startsWith('win') ? "COM?" : CONFIG.serial_energy.port,
    socket_port: CONFIG.socket_energy.port,
    socket_ip: CONFIG.socket_energy.ip
};

const rs485_control_config = {
    portType: CONFIG.portType, //'socket' , 'serial'
    portName: process.platform.startsWith('win') ? "COM?" : CONFIG.serial_control.port,
    socket_port: CONFIG.socket_control.port,
    socket_ip: CONFIG.socket_control.ip
};
...
if (rs485_energy_config.portType == 'serial') {
    ...
    energy_parser = rs485_energy.pipe(new CustomParser());
...
if (rs485_control_config.portType == 'serial') {
    ...
    control_parser = rs485_control.pipe(new CustomParser());
...
energy_parser.on('data', function (data) {
...
control_parser.on('data', function (data) {
    // console.log('Receive interval: ', (new Date().getTime())-lastReceive, 'ms ->', data.toString('hex'));
    lastReceive = new Date().getTime();
