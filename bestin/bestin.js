...
const CONST = {
    ...
    // 상태 Topic (/bestin/${deviceId}${subId}/${property}/state/ = ${value})
    // 명령어 Topic (/bestin/${deviceId}${subId}/${property}/command/ = ${value})
    TOPIC_PRFIX: CONFIG.mqtt.prefix,
    STATE_TOPIC: CONFIG.mqtt.prefix + '/%s%s/%s/state', //상태 전달
    DEVICE_TOPIC: CONFIG.mqtt.prefix + '/+/+/command' //명령 수신
};
...
client.on('connect', () => {
    log("[MQTT] connection successful!", "(" + mqttVar.broker, mqttVar.port + ")");
...
function handleEnergyData(data) {
    ...
    deviceState = CONST.DEVICE_STATE.find(obj => obj.deviceId === 'room');
    ...
    switch (data[3]) {
        case 0x91:
            ...
            // 조명
            deviceState.light1 = (data[6] & 0x01) ? 'ON' : 'OFF';
            deviceState.light2 = (data[6] & 0x02) ? 'ON' : 'OFF';
            deviceState.light3 = (data[6] & 0x04) ? 'ON' : 'OFF';
            deviceState.light4 = (data[6] & 0x08) ? 'ON' : 'OFF';

            // 콘센트/현재전력
            deviceState.outlet1 = (data[7] & 0x01) ? 'ON' : 'OFF';
            deviceState.outlet2 = (data[7] & 0x02) ? 'ON' : 'OFF';
            deviceState.idleCut1 = ((data[8] * 256 + data[9]) / 10).toString(10);
            deviceState.idleCut2 = ((data[10] * 256 + data[11]) / 10).toString(10);
            deviceState.curPower1 = ((data[14] * 256 + data[15]) / 10).toString(10);
            deviceState.curPower2 = ((data[16] * 256 + data[17]) / 10).toString(10);
            deviceState.curPower3 = ((data[18] * 256 + data[19]) / 10).toString(10);
            deviceState.idlePower = ((data.toString('hex')[14]) == 9) ? 'ON' : 'OFF';
...
function handleControlData(data) {
    ...
    switch (data[2]) {
        ...
        case 0x10:
            ...
            deviceCommand = queue.findIndex(
                (e) =>
                    (data[2] === 0x81 | data[2] === 0x82 | data[2] === 0x83 | data[2] === 0x84 | data[2] === 0x87 | data[3] === 0x92) &&
                    data[0] === e.commandHex[0] &&
                    data[1] === e.commandHex[1]
            );
            if (deviceCommand > -1) {
